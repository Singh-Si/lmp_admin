    ...
    const handleSubmit = (e) => {
        ...
        try {
            const hasEmptyFields = rowsData.some((row) => {
                return row.yacht_body_type.trim() === '' || row.location == 0;
            });
            ...
            if (hasEmptyFields) {
                ...
                fetch(`https://lmp.handsintechnology.in/api/add_Yacht_Body_type`, requestOptions)
                    .then(response => response.json())
                    .then(data => {
                        if (data.status == 200) {
                            swal({
                                        ...
                                        rowsData.map((data, index) => {
                                            return (
                                                <tr key={index} >
                                                    ...
                                                    <td>
                                                        <input type="text" onChange={(evnt) => (handleChange(index, evnt))} className="form-control" name="yacht_body_type" placeholder="Yacht Body Type" autoComplete="off" required />
...
export default Addyachtbodytype