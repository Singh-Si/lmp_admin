...
const Updateyachtspeedknotlist = () => {
  ...
  const [yacht_speed_knot_type_location, setYachtspeedknottypelocation] = useState([]);
  ...
  const detailsbyid = async () => {
    ...
    let result = await fetch(`https://lmp.handsintechnology.in/api/get_Yacht_speed_knot_type_detailsbyid`, requestOptions);
    result = await result.json();
    setYachtspeedknottype(result.yacht_speed_knot_type);
    setYachtspeedknottypestatus(result.yacht_speed_knot_type_status);
    const locationid = result.yacht_speed_knot_type_location;
      ...
      fetch(`https://lmp.handsintechnology.in/api/get_location_by_id/${location_id[i]}`, requestOptions)
        ...
        .then((data) => {
          ...
          if (location_name_len === location_id_len) {
            ...
            setYachtspeedknottypelocation(location_name_arr_obj);
          }
        });
    }
    locationList();
  ...
  const updateyachtspeedknot = async (e) => {
    e.preventDefault();
    if (yacht_speed_knot_type === undefined || yacht_speed_knot_type === "" || yacht_speed_knot_type === null) {
      swal("Please Enter Speed knot Type", "", "warning")
      return false;
    }
    if (selectedOption === undefined || selectedOption === "" || selectedOption === null) {
      swal("Please Select Location", "", "warning")
      return false;
    } else if (yacht_speed_knot_type_status === undefined || yacht_speed_knot_type_status === "" || yacht_speed_knot_type_status === null) {
    ...
    let result = await fetch(`https://lmp.handsintechnology.in/api/update_Yacht_speed_knot_type_details`, {
      method: "POST",
      body: JSON.stringify({ ParamValue: ParamValue, yacht_speed_knot_type: yacht_speed_knot_type, yacht_speed_knot_type_location: yacht_speed_knot_type_location_str.toString(), yacht_speed_knot_type_status: yacht_speed_knot_type_status }),
      headers: {
        "Content-Type": "application/json",
      },
    });
    ...
    navigate('/Viewyachtspeedknotlist')
  }
          ...
          <div className="col-md-12">
            <div className="card">
              <div className="card-header">
                <h4 className="card-title">Speed knot Type</h4>
              </div>
                  ...
                  <div className="row">
                    <div className="col-md-6">
                      ...
                      <input type='text' className="form-control"
                        ...
                        onChange={e => setYachtspeedknottype(e.target.value)}
                      />
                    </div>
                    <div className="col-md-6">
                    ...
                    <div className="col-md-6">
                      ...
                      <select className="form-control" name="status" onChange={(e) => setYachtspeedknottypestatus(e.target.value)}>
                        <option defaultValue={yacht_speed_knot_type_status} hidden>{yacht_speed_knot_type_status == 1 ? 'Active' : 'InActive'}</option>
...
export default Updateyachtspeedknotlist