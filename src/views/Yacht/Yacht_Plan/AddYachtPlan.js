...
import TablePlan from './YachtPlanTable';
import { useState } from 'react';
import { useNavigate } from "react-router-dom";
import swal from 'sweetalert';

function AddYachtPlan() {
    ...
    const errorArray = {
        ...
        location: "Location",
        personal_eff_rate: "Personal Effects Rate",
        outboard_range_rate: "Outboard Range Rate",
        din_ten_val_rate: "Dinghy/Tender Value Range Rate",
        trailer_rate: "Trailer Rate",
    }
    ...
    const handleSubmit = async () => {
            ...
            const emptyKeys = Object.keys(rowData).filter((key) => {
                ...
                if (typeof value === 'string' && value.trim() == '') {
                    ...
                    if (key == "company_id" || key == "plan_category_id" || key == "nature_of_plan_id" || key == "plan_name"
                        || key == "initial_rate" || key == "discount_rate" || key == "rate" || key == "excess" || key == "minimum_premium"
                        ...
                        || key == "jdv_commision" || key == "outboard_range_rate" || key == "personal_eff_rate"
                        || key == "din_ten_val_rate" || key == "trailer_rate"
                    ) {
                        return true
                    ...
                    || key == "yacht_hull_material" || key == "yacht_horsepower_type" || key == "yacht_engine_type"
                    || key == "location")) {
        ...
        if (objectsWithEmptyValues.length > 0) {
            ...
            await fetch('https://lmp.handsintechnology.in/api/addyachtplan', requestOptions)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 200) {
                        swal({
                        ...
                        <div className="card-header">
                            <div className="row">
                                <div className="col-md-6">
                                    <h4 className="card-title">Add Comprehensive Yacht Plan</h4>
                                </div>
                                <div className='col-md-6'>
                            ...
                            <table className="table table-bordered" style={{ width: "12000px" }}>
                                <thead>
                                    <tr>
                                        ...
                                        <th>Fixed/Percentage/Reffered (Topup)</th>
                                        <th>Rate</th>
                                        <th>last year policy type</th>
                                        <th>Fixed/Percentage/Reffered (Topup)</th>
                                        <th>Age Of The Boat</th>
                                        <th>Fixed/Percentage/Reffered (Topup)</th>
                                        <th>No Claim year</th>
