    ...
    const handleSubmit = (e) => {
        ...
        try {
            const hasEmptyFields = rowsData.some((row) => {
                return row.make_name.trim() === '' || !row.location;
            });
            ...
            if (hasEmptyFields) {
                ...
                for (let i = 0; i < rowsData.length; i++) {
                    ...
                    fd.append('make_name', rowsData[i].make_name)
                    fd.append('index', i)
                    if (datalen == i) {
                        fd.append('last', 'last')
                    }
                    ...
                    fetch(`https://lmp.handsintechnology.in/api/AddYachtMake`, requestOptions)
                        .then(response => response.json())
                        .then(data => {
                            if (data.status == 200) {
                                swal({
                ...
                <div className="col-md-12" >
                    <div className="card" >
                        <div className="card-header">
                            <div className="row">
                                <div className="col-md-12">
                                    <h4 className="card-title">Add Yacht Make</h4>
                                </div>
                            </div>
                        </div>
                                        ...
                                        rowsData.map((data, index) => {
                                            return (
                                                <tr key={index} >
                                                    ...
                                                    <td>
                                                        <input type="text" onChange={(evnt) => (handleChange(index, evnt))} className="form-control" name="make_name" placeholder="Make Name" autoComplete="off" required />

                                                    </td>
                                                    <td>
                                                        <input type="file" onChange={(evnt) => (handleImageChange(index, evnt.target.files[0]))} className="form-control" name="yacht_make_logo" placeholder="yacht make logo" autoComplete="off" required />
