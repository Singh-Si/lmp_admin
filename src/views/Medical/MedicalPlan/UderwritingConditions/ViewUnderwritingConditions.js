...
const ViewUnderwrintingConditions = () => {
  ...
  const getUnderwritingConditions = async (id) => {
    ...
    await fetch(`https://lmp.handsintechnology.in/api/get_underwriting_conditions`, requestOptions)
      .then(response => response.json())
      .then(data => {
        const UnderwritingCondtions = data.data;
        fetch(`https://lmp.handsintechnology.in/api/single_medical_plan_details/${id}`, requestOptions)
          .then(response => response.json())
          .then(data => {
            const underwriting_conditionDetails = data.data.underwriting_conditions_arr;
            const underwritingArr = [];
            for (let j = 0; j < UnderwritingCondtions.length; j++) {
              for (let i = 0; i < underwriting_conditionDetails.length; i++) {
                if (underwriting_conditionDetails[i].itemId == UnderwritingCondtions[j]._id) {
                  UnderwritingCondtions[j]['value'] = underwriting_conditionDetails[i].value;
                  UnderwritingCondtions[j]['status'] = underwriting_conditionDetails[i].status;
                  UnderwritingCondtions[j]['checked'] = 'checked';
                  underwritingArr.push(UnderwritingCondtions[j]);
                }

              }
            }
            console.log(UnderwritingCondtions, ">>>>>>>>>>>>>>>>>>>>>>> underwriting_conditionDetails")

            setUnderwritingConditions(UnderwritingCondtions);
            setFormData(underwritingArr);
          });
  ...
  const handleSubmit = (e) => {
    ...
    if (hasNoStatus) {
      ...
      fetch(`https://lmp.handsintechnology.in/api/add_underwriting_conditions`, requestOptions)
        .then(response => response.json())
        .then(data => {
          window.location.href = '/ViewUnderwritingConditions?id=' + ParamValue;
...
export default ViewUnderwrintingConditions