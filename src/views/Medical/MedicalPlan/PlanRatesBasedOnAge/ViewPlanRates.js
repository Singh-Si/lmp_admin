...
const ViewPlanRates = () => {
    ...
    const getMedicalBMI = (page, perPage, id) => {
        ...
        fetch(`https://lmp.handsintechnology.in/api/get_rates_based_on_age`, requestOptions)
    ...
    const setStaus = (id, status) => {
        try {
            ...
            fetch(`https://lmp.handsintechnology.in/api/update_medicalplan_rates_status/${id}`, requestOptions)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 200) {
                        swal({
    ...
    const deleteItem = (id) => {
        try {
            ...
            fetch(`https://lmp.handsintechnology.in/api/deleteMedicalMaster?id=${id}&type=rateCondition`, requestOptions)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 200) {
                        swal({
