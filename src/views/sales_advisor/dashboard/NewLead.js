...
function NewLead({ filterOptions, defaultOptions, updateSharedData }) {
  ...
  const getnewleadslist = async (page, perPage) => {
    ...
    if (loginusertype == "6462250eb201a6f07b2dff3a" || loginusertype == "646224eab201a6f07b2dff36"
      || loginusertype == '64622526b201a6f07b2dff3e') {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?leadType=New&dashboardType=salesAdvisorDashboard`, requestOptions)
        ...
        .then(data => {
          ...
          setNewleadData(list)
          console.log("new lead data", list)
        })
        .catch((error) => {
          console.log(error)
        })
    ...
    if (loginusertype == "64622470b201a6f07b2dff22" ||
      (loginusertype == '646224deb201a6f07b2dff32' && SADashboardPermission?.includes('edit'))) {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?leadType=New&dashboardType=salesAdvisorDashboard`, requestOptions)
        .then(response => response.json())
        .then(data => {
          const total = data.total;
  ...
  const getleadstatus = async () => {
    ...
    let result = await fetch('https://lmp.handsintechnology.in/api/get_lead_status_name?type=salesAdvisor', {
      method: 'post',
  ...
  const Documentchaserlist = () => {
    ...
    // fetch(`https://lmp.handsintechnology.in/api/getDocumentChaser`, requestOptions)
    fetch(`https://lmp.handsintechnology.in/api/getUserAccordingUserType?userType=dacumentsChaser`, requestOptions)
      .then(response => response.json())
      .then(data => {
  ...
  const handlesubmit = (e) => {
    ...
    for (const itemId of itemIds) {
      ...
      fetch(`https://lmp.handsintechnology.in/api/update_new_lead_details`, requestOptions)
        .then(response => response.json())
        .then(data => {
          getnewleadslist(page, perPage);
                      ...
                      newleaddata.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td className='buttons_icons'>
                            <button type="button" className="btn btn-primary btn-sm " onClick={() => handleshowlink(item._id)}><i className="fa-solid fa-link"></i></button>
                          </td>
                          <td>
                            <input
                              type="checkbox"
                          ...
                          <td>
                            <select name="agent_list" id={item._id}
                              onChange={(e) => handledcagentchange(item._id, e.target.value)}
                              disabled={leadStatusByItemId[item._id] != 'Forward' ? true : false}
                            >
                              <option value="" hidden>
                                {
                                  // item.forward_to == undefined || item.forward_to == "" || item.forward_to == null ?
                                  'Document Chaser'
                                  // : getDcAgentName(item.forward_to)
                                }
                              </option>
