...
function ClosedBussiness({ filterOptions, defaultOptions }) {
  ...
  const getclosedleadslist = async (page, perPage) => {
    ...
    if (loginusertype == "6462250eb201a6f07b2dff3a" || loginusertype == "646224eab201a6f07b2dff36"
      ...
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?leadType=&dashboardType=salesAdvisorDashboard`, requestOptions)
    ...
    if (loginusertype == "64622470b201a6f07b2dff22") {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?leadType=Closed&dashboardType=salesAdvisorDashboard`, requestOptions)
        .then(response => response.json())
        .then(data => {
          const total = data.total;
          setNotificationCount(total)
...
export default ClosedBussiness;