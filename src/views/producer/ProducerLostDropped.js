...
function ProducerLostDropped({ filterOptions, defaultOptions, updateSharedData }) {
  ...
  const getnewleadslist = async (page, perPage) => {
    ...
    if (loginusertype == "66068569e8f96a29286c956e") {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?producer=LostDropped`, requestOptions)
    ...
    if (loginusertype == "64622470b201a6f07b2dff22") {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?producer=LostDropped&dashboardType=producerDashbord`, requestOptions)
        .then(response => response.json())
        .then(data => {
          const total = data.total;
          setNotificationCount(total)
...
export default ProducerLostDropped;