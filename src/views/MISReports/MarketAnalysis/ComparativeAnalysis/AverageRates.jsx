...
import { AllDataRow, RenderHighestRateRows, Tablehaders } from './RenderAverageRateRows';

AverageRates.propTypes = {
...
function AverageRates({ filterOptions, defaultOptions, activeKey }) {
    ...
    useEffect(() => {
        ...
        if (!token) {
            ...
            activeKey === "15" && getleadslist(state.page, state.perPage);
        }
        ...
        <Accordion.Item eventKey="15">
            <Accordion.Header>
                <div className="card-header new_leads">
                    <strong>Comparative Analysis (Average rate dealt on the website for each type of vehicle (comparison pool))</strong>
                </div>
            </Accordion.Header>
                ...
                <div className='export-texcel-mis'>
                    ...
                    <div>
                        {state.newleaddata.length > 0 && (
                            <div className='export-texcel-mis'>
                                <div>
                                    <button onClick={async () => await MISExcelDownload['AveragerateRate'](state.newleaddata)} className="btn btn-primary">Download</button>
                                </div>
                                <div>
                                    <button
                                        type="button"
                                        className="btn btn-danger"
                                        onClick={() => handlePdfDownload(state.query, "AveragerateRate")}
                                    >
                                        Export to PDF
                                    </button>
                                </div>
                                <div>
                                    <button
                                        type="button"
                                        className="btn btn-danger"
                                        onClick={() => handlePrint(state.query, "AveragerateRate")}
                                    >
                                        Print
                                    </button>
                                </div>
                                ...
                                state.newleaddata && state.newleaddata.length > 0 ? (
                                    state.newleaddata.map((item, index) => (
                                        Object.keys(item).length === 1
                                            ? <RenderHighestRateRows key={index} item={item[state.lobname]} lobname={state.lobname} />
                                            : <AllDataRow key={index} data={item} />
                                    ))
