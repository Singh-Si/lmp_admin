    ...
    const handleSubmit = (e) => {
        ...
        try {
            const hasEmptyFields = rowsData.some((row) => {
                return row.travel_type.trim() === '' || row.location == 0;
            });
            ...
            if (hasEmptyFields) {
                ...
                fetch(`https://lmp.handsintechnology.in/api/add_travel_type`, requestOptions)
                    .then(response => response.json())
                    .then(data => {
                        if (data.status == 200) {
                            swal({
                ...
                <div className="col-md-12" >
                    <div className="card" >
                        <div className="card-header">
                            <div className="row">
                                <div className="col-md-12">
                                    <h4 className="card-title">Add Travel Type </h4>
                                </div>
                            </div>
                        </div>
                                        ...
                                        rowsData.map((data, index) => {
                                            return (
                                                <tr key={index} >
                                                    ...
                                                    <td>
                                                        <input type="text" onChange={(evnt) => (handleChange(index, evnt))} className="form-control" name="travel_type" placeholder="Travel Type" autoComplete="off" required />
