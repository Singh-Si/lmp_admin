...
const ViewGroupMedicalCategories = () => {
    ...
    useEffect(() => {
        ...
        if (token == null || token == undefined || token == '') {
            ...
            GetGroupMedicalCategories(page, perPage);
            getlocationlist();
        }
    }, []);
    ...
    const GetGroupMedicalCategories = async (page, perpage) => {
        const reqOptions = {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
        ...
        fetch(`https://lmp.handsintechnology.in/api/getGroupMedicalCategories?page=${page}&limit=${perpage}`, reqOptions)
            .then(response => response.json())
            .then(data => {
    ...
    const AddGroupMedicalCategory = async (e) => {
        ...
        const reqOptions = {
            ...
            body: JSON.stringify({ category_name, location })
        };
        fetch('https://lmp.handsintechnology.in/api/addGroupmedicalCategory', reqOptions)
            .then(response => response.json())
            .then(data => {
                if (data.status == 201) {
    ...
    const handlePageClick = (data) => {
        ...
        setPage(selected + 1);
        GetGroupMedicalCategories(selected + 1, perPage);
    };
    const ActivateDeactivate = (id, status) => {
        ...
        fetch(`https://lmp.handsintechnology.in/api/updateGroupMedicalCategory?id=${id}`, requestOptions)
            .then(response => response.json())
            .then(data => {
                if (data.status == 200) {
    ...
    const deleteItem = (id) => {
        ...
        fetch(`https://lmp.handsintechnology.in/api/deleteGroupMedicalMaster?id=${id}&type=groupmedicalCategory`, requestOptions)
            .then(response => response.json())
            .then(data => {
                if (data.status == 200) {
                    swal("Success", data.message, "success")
    ...
    const goTosetShowEditModal = (id) => {
        ...
        fetch(`https://lmp.handsintechnology.in/api/getGroupMedicalCategoryById?id=${id}`, reqOptions)
            .then(response => response.json())
            .then(data => {
    ...
    const EditCategory = async (e) => {
        ...
        fetch(`https://lmp.handsintechnology.in/api/updateGroupMedicalCategory?id=${editData?._id}`, reqOptions)
            .then(response => response.json())
            .then(data => {
                if (data.status == 200) {
                        ...
                        <div className='card-header'>
                            <div className='row'>
                                <div className='col-md-4'>
                                    <h4>Group Medical Categories</h4>
                                </div>
                                <div className='col-md-8'>
                                    <button className='btn btn-primary' onClick={() => SetShowAddModal(true)} style={{ float: 'right' }}>Add Category</button>
                                                ...
                                                <div className="card-body">
                                                    <form method='POST' onSubmit={EditCategory}>
                                                        <div className="row">
                                                            <div className='col-lg-6'>
                                                                <label><strong>Category Name</strong></label><br />
                                                                <input type='text' className='form-control' defaultValue={editData?.category_name} name='category_name' />
                                                            </div>
                                                            <div className='col-lg-6'>
                                                                <label><strong>Location</strong></label><br />
                                                                <Multiselect
...
export default ViewGroupMedicalCategories
