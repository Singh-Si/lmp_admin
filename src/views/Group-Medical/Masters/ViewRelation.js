...
const ViewRelation = () => {
  ...
  const getRelation = (page, limit) => {
    ...
    fetch(`https://lmp.handsintechnology.in/api/getRelation?page=${page}&limit=${limit}`, reqOptions)
      .then(response => response.json())
      .then(data => {
        setRelation(data.data)
        console.log(data.data, "Relation data")
      })
  ...
  const AddRelation = (e) => {
    ...
    fetch(`https://lmp.handsintechnology.in/api/addRelation`, reqOptions)
      .then(response => response.json())
      .then(data => {
        if (data.status == 201) {
          getRelation(page, perPage);
  ...
  const goTosetShowEditModal = (id) => {
    ...
    fetch(`https://lmp.handsintechnology.in/api/getRelationbyid?id=${id}`, reqOptions)
      .then(response => response.json())
      .then(data => {
  ...
  const EditRelation = (e) => {
    ...
    fetch(`https://lmp.handsintechnology.in/api/updateRelation?id=${editData?._id}`, reqOptions)
      .then(response => response.json())
      .then(data => {
        if (data.status == 200) {
  ...
  const ActivateDeactivate = (id, status) => {
    ...
    fetch(`https://lmp.handsintechnology.in/api/updateRelation?id=${id}`, requestOptions)
      .then(response => response.json())
      .then(data => {
        if (data.status == 200) {
  ...
  const deleteItem = (id) => {
    ...
    fetch(`https://lmp.handsintechnology.in/api/deleteGroupMedicalMaster?id=${id}&type=Relation`, requestOptions)
      .then(response => response.json())
      .then(data => {
        if (data.status === 200) {
          swal({
  ...
  const handlePageClick = (data) => {
    ...
    setPage(selected + 1);
    getRelation(selected + 1, perPage);
  };
  return (
            ...
            <div className='card-header'>
              <div className='row'>
                ...
                <div className='col-md-8' >
                  <button className='btn btn-primary' onClick={() => SetShowAddModal(true)} style={{ float: 'right' }}>Add Relation</button>
                </div>
            ...
            <div className='card-body'>
              <div className="table-responsive">
                <table className="table table-bordered">
                  ...
                  <tbody>
                    {Relation?.length ? Relation?.map((item, index) => {
                      return (
                        <tr key={index}>
                          <td>{index + 1}</td>
                  ...
                  <div className="row">
                    <div className="col-md-12">
                      <div className="card">
                        ...
                        <div className="card-body">
                          <form method='POST' onSubmit={AddRelation}>
                            <div className="row">
                              <div className='col-lg-6'>
                                <label><strong>Relation</strong></label><br />
                  ...
                  <div className="row">
                    <div className="col-md-12">
                      <div className="card">
                        ...
                        <div className="card-body">
                          <form method='POST'
                            onSubmit={EditRelation}
                          >
                            <div className="row">
                              <div className='col-lg-12'>
                                <label><strong>Relation</strong></label><br />
                                <input type='text' className='form-control' defaultValue={editData?.name} name='Relation' />
                              </div>
                            </div>
                            <div className="row">
                              <div className='col-lg-6'>
