...
function ManagerSalesGraph({ filterOptions, defaultOptions, lobdata }) {
    ...
    const getTotalCount = async () => {
        ...
        if (loginusertype == "64622470b201a6f07b2dff22") {
            await fetch(`https://lmp.handsintechnology.in/api/topLeagentCount?dashboardType=supervisorDashboard`, requestOptions)
                .then(response => response.json())
                .then(data => {
                    console.log("data", data.data);
                    setMotorTotalCount(data.data.motorCount)
    ...
    const getTotaldata = async () => {
        ...
        if (loginusertype == "64622470b201a6f07b2dff22") {
            await fetch(`https://lmp.handsintechnology.in/api/getAllGraphCount?dashboardType=supervisorDashboard`, requestOptions)
                .then(response => response.json())
                .then(data => {
                    console.log("data", data.data.sepmotorCount);
                    setJanmotorCount(data.data.janmotorCount);
