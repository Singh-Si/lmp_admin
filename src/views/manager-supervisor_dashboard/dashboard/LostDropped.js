...
function LostDropped({ filterOptions, defaultOptions }) {
  ...
  const getleadslist = async (page, perPage) => {
    ...
    if (loginusertype == "646224deb201a6f07b2dff32") {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?assignType=lostAndDropped&dashboardType=supervisorDashboard`, requestOptions)
  ...
  const handleassignedsubmit = async (leadid) => {
    ...
    if (loginusertype == "6462250eb201a6f07b2dff3a" || loginusertype == "646224eab201a6f07b2dff36"
      ...
      await fetch(`https://lmp.handsintechnology.in/api/reassigned_agent?status=new?dashboardType=MultiSupervisorDashboard`, requestOptions)
        .then((response) => response.json())
        .then((data) => {
          if (data.status == 200) {
            alert(data.message)
    ...
    if (loginusertype == "64622470b201a6f07b2dff22") {
      await fetch(`https://lmp.handsintechnology.in/api/reassigned_agent?dashboardType=supervisorDashboard&status=new`, requestOptions)
        .then(response => response.json())
        .then(data => {
          if (data.status == 200) {
            alert(data.message)
