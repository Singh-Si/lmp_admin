...
function BEHotleads({ filterOptions, defaultOptions, updateSharedData }) {
  ...
  const getnewleadslist = async (page, perPage) => {
    ...
    if (loginusertype == "646224eab201a6f07b2dff36") {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?leadType=Hot`, requestOptions)
    ...
    if (loginusertype == "64622470b201a6f07b2dff22") {
      await fetch(`https://lmp.handsintechnology.in/api/get_supervisor_leads?leadType=Hot&dashboardType=salesAdvisorDashboard`, requestOptions)
        .then(response => response.json())
        .then(data => {
          const total = data.total;
          const slice = total / perPage;
